...
export default function CopyrightCreator({ image, filename, uid, setUid, onStart, onProgress, onEnd }: Props) {
  ...
  const injectData = async () => {
    ...
    if (testResult && testResult.uid === uid.toUpperCase()) {
      await bundleEvidence(canvas.toDataURL('image/png'), borderCanvas.toDataURL('image/png'), interiorCanvas.toDataURL('image/png'), deed, `${uid}_${filename}`, (p: number) => onProgress(p));
